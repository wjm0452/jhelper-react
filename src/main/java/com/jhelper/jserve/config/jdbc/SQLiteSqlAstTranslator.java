package com.jhelper.jserve.config.jdbc;
...
public class SQLiteSqlAstTranslator<T extends JdbcOperation> extends AbstractSqlAstTranslator<T> {
    ...
    protected boolean shouldEmulateFetchClause(QueryPart queryPart) {
        // Check if current query part is already row numbering to avoid infinite
        // recursion
        // We also have to emulate this if a fetch clause type other than rows only is
        // used
    ...
    protected void renderPartitionItem(Expression expression) {
        if (expression instanceof Summarization) {
            // This could theoretically be emulated by rendering all grouping variations of
            // the query and
            // connect them via union all but that's probably pretty inefficient and would
            // have to happen
